...
const sendMessage = async (req, res) => {
    try {
        ...
        if (detectado) {
            console.log(`🚨 ALERTA ROJA: Posible desvío de cliente por ${sent_by}`);
        }

        // Only send WhatsApp for real text messages (not call logs or system notes)
        ...
        // Always save to DB regardless of Twilio result
        const result = await pool.query(
            ...
            [lead_id, message || null, sent_by, business || null, type, call_result || null, call_duration || null]
        );
