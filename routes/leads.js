...
// --- RUTAS DE LEADS ---
// Esta es la ruta principal que el War Room busca para mostrar los leads activos
router.get('/', getActiveLeads); 
...
router.post('/send-message', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO messages (lead_id, message_text, direction, sent_by, message_type, call_result, call_duration) VALUES ($1,$2,\'outgoing\',$3,$4,$5,$6) RETURNING *',
...
router.get('/unread-count', async (req, res) => {
  try {
    const alba = await pool.query("SELECT COUNT(*) FROM leads WHERE status = 'nuevo' AND (category = 'alba' OR category IS NULL)");
    const huellitas = await pool.query("SELECT COUNT(*) FROM